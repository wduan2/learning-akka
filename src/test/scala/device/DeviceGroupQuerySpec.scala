...
class DeviceGroupQuerySpec extends FlatSpec
  with Matchers {
  ...
  "A DeviceGroupQuery actor" should "return TemperatureNotAvailable for devices with no readings" in {
    ...
    requester.expectMsg(
      DeviceGroup.RespondAllTemperatures(
        requestId = 1,
        temperatures = Map("device1" -> DeviceGroup.TemperatureNotAvailable, "device2" -> DeviceGroup.Temperature(2.0))
      )
    )
  }

  "A DeviceGroupQuery actor" should "return DeviceNotAvailable if devices stops before answering" in {
    val requester = TestProbe()
    val device1 = TestProbe()
    val device2 = TestProbe()
